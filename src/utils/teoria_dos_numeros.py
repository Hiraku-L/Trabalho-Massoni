...
def primo_de_blum(bits: int):
    while True:
        resp = getPrime(bits)
        if resp % 4 == 3:
            return resp

def simbolo_legendre(a: int, p: int):
    valor = pow(a, (p-1) // 2, p)
    if valor == p-1:
        return -1
    return valor
...
def nao_residuo_quadratico(p: int, q: int, n: int):
    while True:
...
def bytes_para_bits(bits: list[int]):
    if len(bits) % 8 != 0:
        raise ValueError("A quantidade de bits deve ser multiplo de 8.")
    resp = bytearray()
